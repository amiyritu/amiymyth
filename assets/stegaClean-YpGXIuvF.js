...
function s(e) {
  ...
  return `${u}${Array.from(r)
    .map((n) => {
      let t = n.charCodeAt(0);
      if (t > 255)
        throw new Error(
          `Only ASCII edit info can be encoded. Error attempting to encode ${r} on character ${n} (${t})`
        );
      return Array.from(t.toString(4).padStart(4, "0"))
        .map((c) => String.fromCodePoint(o[c]))
        .join("");
    })
    .join("")}`;
}
